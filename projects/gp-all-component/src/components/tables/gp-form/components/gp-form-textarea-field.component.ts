...
export class GpFormTextAreaFieldComponent extends GpFormFieldControl implements OnInit {
  ...
  copyValueFromEditedRowToControl(editedRow: any) {
    ...
    if (this.formField.fieldMetadata.displayInfo.translationInfo != null
  ...
  validateField(editedRow: any) {
    ...
    if (!this.formField.fieldMetadata.allowAscii) {
      ...
      if (/[\u0080-\uFFFF]/.test(valorCampo)) {
        this.formField.validField = false;
        this.validateFieldAddMsgs(
          'El valor indicado contiene caracteres no válidos (acentos, eñes ...). Han sido sustituidos por caracteres equivalentes o descartados. Seleccione guardar otra vez para aceptar los cambios.');
