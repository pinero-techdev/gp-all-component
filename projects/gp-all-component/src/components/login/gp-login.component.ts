...
import {LoginRq, LoginService} from '../../services/login.service';
import {GpMainMenuComponent} from '../menu/gp-main-menu.component';

@Component({
  selector: 'gp-login',
  templateUrl: './gp-login.component.html',
  providers: [GpMainMenuComponent]
})
export class GpLoginComponent implements OnInit, OnDestroy {
  usuario: string;
  password: string;
  url: string;
  msgs: Message[] = [];
  ...
  constructor(private router: Router, private _loginService: LoginService,
              private _gpAppMainMenu: GpMainMenuComponent, private _route: ActivatedRoute) {
  ...
  login(urlToRedirect?: string, otherParams?: string) {
    this.working = true;
    const request: LoginRq = new LoginRq(this.usuario, this.password, GlobalService.getAPLICACION_LOGIN(),
      GlobalService.getPARAMS_LOGIN(), otherParams);
    this._loginService.login(request).pipe(finalize(() => {
      this.working = false;
    })).subscribe(
      data => {
        if (data.ok) {
          ...
          } else if (!(GlobalService.getPRE_LOGIN_URL() === null || GlobalService.getPRE_LOGIN_URL() === undefined)
