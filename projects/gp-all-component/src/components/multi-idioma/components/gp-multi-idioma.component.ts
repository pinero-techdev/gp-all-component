...
export class GpMultiIdiomaComponent extends Mensajes implements OnInit {
  ...
  guardarCambios() {
    ...
    for (const traduccionesInsertar of this.elementosTraducciones) {
      const request = new UpdateTraduccionesRq(this.pKey, this.esquema, this.tabla, this.campo,
        traduccionesInsertar.codigoIdioma,
        traduccionesInsertar.idiomaPaisTraduccion);
      this._multiIdiomaService.actualizaTraducciones(request)
        .pipe(finalize(() => {
          this.working = false;
        })).subscribe(
        data => {
  ...
  contieneHtml(traduccion: string): boolean {
    return traduccion != null && (traduccion.indexOf('</') !== -1 || traduccion.indexOf('/>') !== -1 || traduccion.indexOf('&lt;') !== -1
  ...
  mostrarDialogoEdicionHTML(traduccion: Traduccion) {
    this.textoHTML = new Traduccion(traduccion.codigoIdioma, traduccion.idiomaPais,
