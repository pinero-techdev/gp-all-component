import { GPSelectItem } from './gp-select-item.model';
...
export class GPSelector {
     ...
     *                                 atributos de la etiqueta del options. Por defecto es '-'
     * @param autoSeleccion - Indica si se tiene que seleccionar por defecto un valor,
     *                        en caso de que solo haya una opción disponible
    ...
    public cargarDatos(
        ...
        if (
            ...
            (!datos || (datos != null && datos.length !== 1) || !autoSeleccion)
        ) {
            if (translate) {
                this.options.push({
                    label: translate.transform(descripcionPorDefecto),
                    value: null,
                });
            } else {
        ...
        if (datos != null) {
            ...
            if (autoSeleccion && datos.length === 1) {
                this.selection = this.isMultiSelect
