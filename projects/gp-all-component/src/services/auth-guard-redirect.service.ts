...
export class AuthGuardRedirect implements CanActivate {
   ...
   *   component: NOMBRE_DEL_COMPONENTE, canActivate: [AuthGuardRedirect], data: { menuOptionIds: ['CODIGO_OPCION','OTRO_CODIGO_OCPION']}
   *  }
          ...
          .pipe(map(
            menu => {
              if (menu) {
                ...
                const accesoPermitido = this._menuAppMenuProviderService.tieneOpcionesMenuActivas(menu, menuOptions);
                if (!accesoPermitido) {
