...
export class CommonService {
  ...
  postNoAuth<T>(url: string, body: any, headers: HttpHeaders =
    new HttpHeaders({'Content-Type': 'application/json; charset=UTF-8'})): Observable<T> {
    const options = new RequestOptions(headers);
    return this.http.post<T>(url, body, options);
  }
  ...
  getNoAuth<T>(url: string, headers: HttpHeaders = new HttpHeaders({'Content-Type': 'application/json; charset=UTF-8'})): Observable<T> {
    const options = new RequestOptions(headers);
    return this.http.get<T>(url, options);
  }
