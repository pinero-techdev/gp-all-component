...
@Injectable()
export class ExtendLoginService {
  ...
  sessionInfo(): Observable<SessionInfoRs> {
    ...
    if (!GlobalService.SESSION_ID) {
      if (sessionStorage.getItem('userInfo') != null) {
        ...
        GlobalService.setSession(JSON.parse(localStorage.getItem('userInfo')));
      }
      if (sessionStorage.getItem('sessionId') != null) {
        GlobalService.setSessionId(sessionStorage.getItem('sessionId'));
      } else if (localStorage.getItem('sessionId') != null) {
        GlobalService.setSessionId(localStorage.getItem('sessionId'));
      }
    }
    if (GlobalService.SESSION_ID) {
      ...
      return this.http.post<SessionInfoRs>(url, sessionInfoRq, options).map(sessionInfoRs => {
        if (sessionInfoRs.ok) {
          ...
          localStorage.setItem('userInfo', JSON.stringify(sessionInfoRs.userInfo));
          localStorage.setItem('sessionId', sessionInfoRs.sessionId);
        }
